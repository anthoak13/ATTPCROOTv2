 ...
 * MCFitter namespace requires ProcessEvent() to be thread safe when multuple threads are
 * calling that function at once.
 */
class AtClusterize {
protected:
   ...
   double fDetPadPlane{};                   //!< Position of the pad plane with respect to the entrance [mm]
      ...
      // Memory that cannot be shared across threads
...
public:
   ...
   std::vector<SimPointPtr> ProcessEvent(const std::vector<AtMCPoint> &fMCPointArray);
...
protected:
   virtual std::vector<SimPointPtr> processPoint(const AtMCPoint &mcPoint, int pointID = -1);
