...
ATTPC2Body::ATTPC2Body(const char* name,std::vector<Int_t> *z, std::vector<Int_t> *a, std::vector<Int_t> *q,
		       ...
		       std::vector<Double_t> *mass, std::vector<Double_t> *Ex, Double_t ResEner,
		       Double_t MinCMSAng, Double_t MaxCMSAng)
  : fMult(0),
    fPx(0.), fPy(0.), fPz(0.),
    fVx(0.), fVy(0.), fVz(0.),
...
Bool_t ATTPC2Body::ReadEvent(FairPrimaryGenerator* primGen) {
  ...
  if(gATVP->GetEnergy() > 0 && !gATVP->IsBeamEvt() )
  {
    ...
    //fWm is the total mass
    //Mystery variable names for kinematics
	...
	Ang.push_back(TMath::ATan(tg_thetalabr));
      ...
      //This will never be called already checked in the parent if statement
      if(!gATVP->GetValidKine())
      {
	...
	if(fPType.at(i)=="Ion")
	  ...
	  std::cout << "-W- FairIonGenerator: Particle " << fParticle.at(i)->GetName()
		    << " not found in database!" << std::endl;
	...
	return kFALSE;
      ...
      if(i > 1 && !gATVP->IsBeamEvt())
      {
	if( pdgType != 1000500500 && fPType.at(i)=="Ion" )
	{
