//Code to simulate the transport of 195Tl beam through 1 bar H gas

void Tl195_sim(Int_t nEvents = 10, TString mcEngine = "TGeant4")
{

  TString dir = getenv("VMCWORKDIR");

  // Output file name
  TString outFile ="./data/Tl195_sim.root";

  // Parameter file name
  TString parFile="./data/Tl195_simPar.root";
 ...
 // ------------------------------------------------------------------------
  ...
  // ---- Beam ----
  Zp.push_back(z); // 195Tl TRACKID=0
  Ap.push_back(a); //
  Qp.push_back(q);
  Pxp.push_back(px);
  Pyp.push_back(py);
  ...
  // //--- Scattered -----
  // Zp.push_back(z); // 195Tl TRACKID=1
  // Ap.push_back(a); //
  // Qp.push_back(q);
  // Pxp.push_back(0.0);
  // Pyp.push_back(0.0);
  ...
  //Create a 2body generator?
  ATTPC2Body* TwoBody = new ATTPC2Body("TwoBody", &Zp, &Ap, &Qp, mult,
				       &Pxp, &Pyp, &Pzp, &Mass, &ExE,
