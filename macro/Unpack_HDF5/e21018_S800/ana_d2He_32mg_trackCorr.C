...
void ana_d2He_32mg_trackCorr(Int_t runNumber)
{
   ...
   TString digiFileName = TString::Format("/mnt/analysis/e21018/rootMerg/Zarif/run_2%03d_%04d.root", runNumber, runNumber);
   ...
   TFile *outfile;
   TString outFileNameHead = TString::Format("/mnt/analysis/e21018/rootAna/run_2%03d_%04d_trackCorr.root", runNumber, runNumber);
   ...
   Double_t lowestDistLines =0;
   Double_t lowestAvgChi2 =0;
   Double_t BestEx = 0, BestVertexZ = 0, BestPosZ = 0, BestMinDist = 0, BestChi2 = 0;
   Double_t bestC = 0.00;
   Double_t chi2Fit1 = 0, chi2Fit2 = 0;
   ...
   anatree->Branch("BestC",&bestC);
   anatree->Branch("BestEx",&BestEx);
   anatree->Branch("BestChi2",&BestChi2);
   
   Float_t distLinesNew=0, pPairSize=0, chargeTotNew1=0, chargeTotNew2=0, range_p1_new=0, range_p2_new=0;
   Int_t InCondition13C=0, InCondition12C=0;
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      if (patternEvent) {
         ...
         for (size_t ive = 0; ive < NVtxEvt; ive++) {
            ...
            Double_t chi_square_1, chi_square_2;
            ...
            lowestAvgChi2 = std::numeric_limits<double>::max();
			Double_t c;
			bestC = 0.0;
            for (c=bestC; c <= 0.0001; c += 0.00001) { 
            ...
            for (size_t itv = 0; itv < NTracksVtx; itv++) {
               ...
               for (Int_t iHit = 0; iHit < hitArray.size(); iHit++) {
                  ...
                  position.X() = TMath::Cos(TMath::ATan2(position.Y(),position.X()))*(TMath::Sqrt(position.X()*position.X()+position.Y()*position.Y())+ (c)*position.Z()*position.Z()/(TMath::Sqrt(position.X()*position.X()+position.Y()*position.Y())));
                  position.Y() = TMath::Sin(TMath::ATan2(position.Y(),position.X()))*(TMath::Sqrt(position.X()*position.X()+position.Y()*position.Y())+ (c)*position.Z()*position.Z()/(TMath::Sqrt(position.X()*position.X()+position.Y()*position.Y())));
            ...
            chi2Fit1 = ransacLine1->GetChi2(); //new line for correction
            chi2Fit2 = ransacLine2->GetChi2(); //new line for correction
            
            double avgChi2 = (chi2Fit1 + chi2Fit2) / 2.0;
						
						if (avgChi2 < lowestAvgChi2) {      
                            lowestAvgChi2 = avgChi2;
                            bestC = c;
                        }
           ...
           // std::cout << " check values, Ex  " << Ex4 << " theta_cm " << theta_cm << " epsilon_pp " << epsilon_pp << " " << std::endl;
            ...
            std::cout<<"Run Number= "<<irun<<"Event Number= "<<ivt<<" Best Chi2 Avg= "<<lowestAvgChi2<<" Best C= "<<bestC<<" Ex= "<<Ex4<<" Best Ex= "<<BestEx<<std::endl  ;
         } // for tv size (ive)
      }    // RANSAC null pointer
   }       // Event loop
