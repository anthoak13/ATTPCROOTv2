...
void PIDS800Corr(int runNumberS800){
  ...
  TString digiFileName = TString::Format("/mnt/analysis/e21018/rootS800/cal/run-%04d-00.root", runNumberS800);
  ...
  for(Int_t i=0;i<nEvents;i++){
   ...
   // Double_t S800_tof = S800_timeObj - S800_timeE1;
    ...
    Double_t ObjCorr1C1 = 105.; //70
    Double_t ObjCorr1C2 = 0.015; //0.0085
...
//----------- New 10/01 -------------------------------------
    ...
    for(int k=0; k<S800_timeMTDCXf.size(); k++){
    	if(S800_timeMTDCXf.at(k)>100 && S800_timeMTDCXf.at(k)<210) S800_timeXfSelect=S800_timeMTDCXf.at(k);//100 210
    }
    for(int k=0; k<S800_timeMTDCObj.size(); k++){
    	if(S800_timeMTDCObj.at(k)>-300 && S800_timeMTDCObj.at(k)<-100) S800_timeObjSelect=S800_timeMTDCObj.at(k);//-300 -100
    }
